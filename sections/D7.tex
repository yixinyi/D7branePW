...
The holographic dictionary for $N_f$ flavors of quarks in a four-dimensional $SU(N)$ SYM theory is a set of $N_f$ D7-branes in the ten-dimensional supergravity dual. We work in the probe limit, when $N_f \ll N$, meaning the additional branes do not back-react on the background geometry. 
We study the simplest setting, with only $N_f=1$ probe brane. Our D7-brane wraps the warped $AdS_5$ and the three-dimensional ellipsoid of the deformed $S^5$ of the Pilch-Warner metric. Furthermore, our D7-brane carries no charge, hence no worldvolume gauge field: $F = 0$. This is the equivalent setting studied in \cite{Karch:2002sh} for $AdS_5 \times S^5$, which our configuration will reduce to, near the boundary.
...
\end{equation}
In this case, the induced metric from \eqref{eq:PWmetric} with $d\theta = \theta'(c) dc$ and $d\phi=0$ is:
...
Let us focus on the $P[C_{(8)}]$ term, which was considered as vanishing in \cite{Albash:2011nw} and \cite{Evans:2005ti}. It is important to consider the string frame metric in the Hodge star operation while deriving $C_{(8)}$, which in our scenario is
\begin{equation}
 dC_{(8)} = \ast dC_{(0)}.
\end{equation}
...
The action is divergent near the boundary since its geometry is asymptotically AdS. The divergent terms are:
\begin{align}
